#!/usr/bin/env bash
# Input ablations (Q-only / C-only / mismatched) for the tuned SLM. Runs inside
# the vllm container. Usage: bash scripts/eval_ablations.sh
set -euo pipefail
...
docker run --rm --gpus all \
  ...
  "$IMAGE" -c "
    ...
    python3 -u -m eval.run_eval_ablations $*
  "
