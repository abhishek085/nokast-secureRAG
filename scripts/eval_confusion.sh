#!/usr/bin/env bash
# Confusion matrix + per-class + CI for B3 on the held-out test set. Runs
# inside the vllm container. Usage: bash scripts/eval_confusion.sh
set -euo pipefail
...
docker run --rm --gpus all \
  ...
  "$IMAGE" -c "
    ...
    python3 -u -m eval.run_eval_confusion $*
  "
