#!/usr/bin/env bash
# External-benchmark generalization check: PoisonedRAG targeted attacks against
# our (Q,C) detector. Runs inside the vllm container. Usage: bash scripts/eval_poisonedrag.sh
set -euo pipefail
...
docker run --rm --gpus all \
  ...
  "$IMAGE" -c "
    ...
    python3 -u -m eval.run_eval_poisonedrag $*
  "
