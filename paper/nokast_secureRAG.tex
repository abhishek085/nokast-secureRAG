...
trusted~\cite{greshake,agentpoison,hipt}. Existing defenses are either
retrieval-centric (sanitization) or safety-centric guards trained on direct
jailbreaks rather than RAG context consistency~\cite{hipt,llamaguard}. We present
...
pipeline: a 35B teacher generates 5{,}233 realistic $(Q,C)$ records across five
...
for a keyword filter, directly demonstrating context-aware detection. The model runs
at $37$\,ms median latency on a single consumer-class device. All results are
in-distribution; external-benchmark generalization is identified as future work.
...
knowledge while keeping data on user-controlled hardware~\cite{hipt}. Integrating
external sources, however, exposes the system to prompt injection and data
poisoning~\cite{greshake,agentpoison,hipt}. Unlike direct jailbreaks, \emph{indirect}
...
boundary: any content that can be retrieved can influence generation. The HiPT/%
OpenRAG-Soc harness standardizes end-to-end RAG evaluation from ingestion through
retrieval to answer generation~\cite{hipt}, which we adopt conceptually as the
...
Benchmarks increasingly stress-test IPI in realistic carriers. Guo and Wei's HiPT
embeds injections in hidden HTML, alt text, and accessibility attributes and ships
deployable mitigations such as sanitization, Unicode normalization, and
attribution-gated answering~\cite{hipt}; Yi et al. (BIPIA) benchmark indirect injection
...
instructions~\cite{hipt}. These efforts largely target either ingestion mechanics or
...
sanitization~\cite{hipt}, generic safety guards~\cite{llamaguard}, and attack/poisoning
...
Teacher (35B) $\xrightarrow{\text{generate }(Q,C)+\text{label}}$ raw data
$\;\rightarrow\;$ Judge (120B, blind relabel) $\xrightarrow{\text{agree/dispute}}$
agreed data $\;\rightarrow\;$ Student (0.5B + LoRA) $\;\rightarrow\;$ semantic firewall}}
...
The teacher generates five attack families plus benign and ambiguous cases:
indirect prompt injection (\textsf{ipi}), knowledge-base poisoning, context hijacking
(role/permission redefinition), PII exfiltration, and \textsf{borderline} (ambiguous
instruction-like text). For each record the attack payload is embedded in $C$ (e.g.,
...
($4{,}973$ agreed, $260$ disputed, $0$ without a verdict). The student is trained on
the $4{,}973$ agreed records ($3{,}835$ train / $479$ validation); the test set
comprises $740$ held-out records including $120$ intact judgment-flip pairs.
...
Table~\ref{tab:main} reports the held-out test set (judge-verified labels). The tuned
...
\begin{table}[t]
\centering
\caption{Detection on the held-out test set (judge-verified labels). $\uparrow$/$\downarrow$
indicate better direction.}
\label{tab:main}
\begin{tabular}{lccccc}
\toprule
System & Recall\,$\uparrow$ & ASR-proxy\,$\downarrow$ & FPR\,$\downarrow$ &
3-class acc\,$\uparrow$ & latency p50 \\
\midrule
...
The tuned SLM runs at $37$\,ms median ($57$\,ms p95) per $(Q,C)$ call under batched
evaluation on the GB10---well within the interactive sub-$100$\,ms target. Single-call
eager latency is $\sim$$400$\,ms and would improve under an optimized serving runtime.
End-to-end the pipeline is cheap: data generation drew $\sim$$35$\,Wh, fine-tuning
$\sim$$9$\,Wh ($14.6$ minutes, $94\%$ GPU utilization), and the final adapter is
$17.6$\,MB. The entire teacher$\rightarrow$judge$\rightarrow$student pipeline runs on a
single consumer-class device.
...
establish robustness on external benchmarks (e.g., HiPT/OpenRAG-Soc~\cite{hipt}) or in
...
The tuned SLM reaches $0.994$ recall at $0.026$ FPR and $37$\,ms latency, and---most
importantly---solves context-dependent ``judgment flip'' cases that keyword baselines
cannot. External-benchmark generalization, true downstream-ASR measurement, and
adaptive-adversary stress testing are the focus of the next iteration of this work.
...
\bibitem{hipt} H. Guo and Z. Wei, ``Hidden-in-plain-text: A benchmark for social-web
